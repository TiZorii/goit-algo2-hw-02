...
def optimize_printing(
    ...
    sorted_print_jobs = sorted(print_jobs, key=lambda job: job.priority)
    ...
    for job in sorted_print_jobs:
        # Перевірка на обмеження по max_volume та max_items
        pass_constraints = (
            volumes + job.volume <= constraints.max_volume
            and counts < constraints.max_items
        )

        if pass_constraints:
            print_order.append(job.id)
            total_time = max(total_time, job.print_time)
            volumes = volumes + job.volume
            counts += 1
        else:
            print_order.append(job.id)
